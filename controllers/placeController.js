...
const getPlacesByUserId = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "เกิดข้อผิดพลาดขณะดึงข้อมูล", error: error.message });
  }
};
...
const getPlaceByUserIdAndPlaceId = async (req, res) => {
  ...
  if (!userId || !placeId) {
    return res
      .status(400)
      .json({ message: "userId และ placeId จำเป็นต้องระบุ" });
  }
  ...
  try {
    ...
    if (!doc.exists) {
      return res
        .status(404)
        .json({ message: "ไม่พบสถานที่ที่ตรงกับ placeId นี้" });
    }
    ...
    if (data.userId !== userId) {
      return res
        .status(404)
        .json({ message: "สถานที่นี้ไม่ได้อยู่ในบัญชีผู้ใช้นี้" });
    }
...
const updatePlace = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "เกิดข้อผิดพลาดขณะอัปเดตข้อมูล", error: error.message });
  }
};
...
const deletePlace = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "เกิดข้อผิดพลาดขณะลบข้อมูล", error: error.message });
  }
};
